	...
	,	setExtraListFilters: function ()
		{
			...
			/*if (this.data.serialnumber) {
				this.filters.serialnumber_operator = 'and';
				this.filters.serialnumber = ['item.itemid', 'contains', this.data.serialnumber];
			}*/


			if (this.data.serialnumber) {

	                var lineCasesServiceURL = nlapiResolveURL(
	                        'SUITELET',
	                        'customscript_sc_get_serial_look_up',
	                        'customdeploy_sc_get_serial_look_up',
	                        true
	                    ) + '&serialnumber=' + this.data.serialnumber + '&types=' + this.data.types + "&customer=" + nlapiGetUser()
	                    ,	lineCasesResponse   =	nlapiRequestURL( lineCasesServiceURL )
	                    ,	invoices = JSON.parse(lineCasesResponse ? lineCasesResponse.getBody() : '[]');

	                if(invoices.length > 0) {
	                    this.filters.serialnumber_operator = 'and';
	                    this.filters.serialnumber = ['internalid', 'anyof', invoices];
	                }else{
